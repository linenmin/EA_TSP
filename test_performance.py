...
@njit(cache=True, fastmath=True)
def _construct_abcycle_atsp(start, succA, predB, u_buf, v_buf, max_steps, visited_mask):
    """构造 AB-cycle，增加 visited 检查防止形成非简单环（8字形）"""
    n = len(succA)
    u = start
    cycle_len = 0    
    for step in range(max_steps):
        if visited_mask[u] == 1 and u != start:
            return -1  # 构成了非简单环，失败
        visited_mask[u] = 1  # 标记访问        
        v = succA[u]
...
def _eax_lite_atsp_inplace(pA, pB, D_eval, finite_mask, knn_idx, child,
    ...
    for attempt in range(MAX_M):
        ...
        # 【修复 Issue 2】清理 visited_mask，防止 8 字形非简单环
        mark_buf[:] = 0        
        cycle_len = _construct_abcycle_atsp(start, succA_buf, predB_buf, 
                                            temp_u_buf, temp_v_buf, max_cycle_steps, mark_buf)
        ...
        for t in range(1, n):
            ...
            for k in range(K):
                ...
                if used[j] == 0 and finite_mask[cur, j]:
                    tmp_idx[cnt], tmp_dis[cnt] = j, D[cur, j]
                    cnt += 1
            nxt = -1
            if cnt == 0:
                best_dist = 1e20
                for j in range(n):
                    if used[j] == 0 and finite_mask[cur, j] and D[cur, j] < best_dist: 
                        best_dist, nxt = D[cur, j], j
                if nxt == -1:
                    for j in range(n):
                ...
                for i in range(limit):
                    ...
                    for j in range(i + 1, cnt):
                        if tmp_dis[j] < tmp_dis[min_idx]: min_idx = j
                    tmp_dis[i], tmp_dis[min_idx] = tmp_dis[min_idx], tmp_dis[i]
                    tmp_idx[i], tmp_idx[min_idx] = tmp_idx[min_idx], tmp_idx[i]
                nxt = int(tmp_idx[np.random.randint(0, limit)])
            if nxt != -1 and not finite_mask[cur, nxt]:
                valid_tour = False
                break
            tour[t], used[nxt], cur = nxt, 1, nxt
        if valid_tour and finite_mask[tour[n - 1], tour[0]]:
            return tour
