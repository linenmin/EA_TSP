...
class r0123456:
    ...
    def _k_tournament(self, population: List[np.ndarray], fitness: np.ndarray, k: int) -> np.ndarray:
        # Probabilities proportional to fitness (as requested).
        # Note: with minimization this favors worse tours. Keep as specified.
        ...
        # Precompute CDF for roulette draws
        cdf = np.cumsum(probs)
        cdf[-1] = 1.0  # guard against fp error

        # Draw k candidates by roulette (with replacement)
        ...
        # Tournament: return the best (minimum fitness) among the k draws
        best_idx = min(chosen_idxs, key=lambda i: fitness[i])
...
if __name__ == "__main__":
    ea = r0123456(N_RUNS=1000, lam=20000, mu=4000)
