...
| **最佳成绩** | ~100224 | 5 分钟内 |
| **种群大小** | λ=200 | 平衡多样性与速度 |
| **代数/5min** | ~10300 (Exploiter) | 非常快 |
| **收敛特征** | 快速收敛，频繁触发排斥 | Explorer 多次被 Repulsion |

**运行特点**:
- ✅ 代数很高，搜索充分
- ⚠️ 后期 Exploiter 和 Explorer 频繁收敛到同一解，触发大量 Repulsion
- ⚠️ Final Polish 未找到改进（可能需要更强的扰动）

**改进方向**:
1. 考虑增加种群大小以提高多样性
2. 调整 Repulsion 阈值，减少无效重启
...
| **最佳成绩** | ~119208 | λ=200 时 |
| **种群大小** | λ=200 | ⭐ 验证最佳 |
| **代数/5min** | ~660 (Exploiter) | 中等 |
| **收敛特征** | 持续改进 | 较好 |

**运行特点**:
- ✅ λ=200 是最佳配置（λ=150 退步到 119837）
- ✅ 代数足够，能持续发现改进
- ⚠️ 仍然存在频繁 Repulsion (Dist: 0.0)

**关键教训**:
- **λ=150 退步**: 种群太小导致多样性不足，虽然代数更多但收敛到更差的局部最优
- **stagnation_limit=200**: 当前配置，Exploiter 实际阈值 = 200 × 0.8 = 160 代

**改进方向**:
1. 探索 λ=250 是否更好
2. 考虑实现 P4 (远交配偶选择) 进一步提升多样性
...
**改进方向**:
1. 考虑增加 Or-Opt 的 max_iters 参数
2. 尝试更小的种群 (λ=80) 换取更多代数
3. 实现 P5 (Don't Look Bits) 加速局部搜索
...
1. **Final Polish 无效**
   - 现象: Double Bridge + 2-Opt 几乎从不找到改进
   - 原因: 2-Opt 在非对称 TSP 上 Delta 计算错误
   - 解决: 将 Final Polish 改为 Double Bridge + Candidate-Or-Opt

2. **Repulsion 过于频繁**
   - 现象: Explorer 频繁触发排斥重启 (Dist: 0.0)
   - 原因: 两岛种群快速收敛到相同解
   - 解决: 考虑降低 Repulsion 阈值（从 5% 降到 3%）或使用 Negative Assortative Mating

### 中优先级

3. **种群规模敏感性**
   - 现象: λ=150 退步，λ=200 最佳
   - 说明: 存在一个最佳种群规模，太大浪费计算，太小多样性不足
   - 建议: 通过 HPC 超参搜索确定各问题的最佳 λ

4. **Exploiter/Explorer 速度差异**
   - 现象: Exploiter 代数远高于 Explorer (因为 Exploiter 局部搜索更少)
   - 说明: 这是预期行为，但需要监控两者的实际贡献

### 低优先级

5. **P4: Negative Assortative Mating**
   - 状态: 未实现
   - 预期收益: 从源头注入多样性，减少收敛速度

6. **P5: Don't Look Bits**
   - 状态: 未实现
   - 预期收益: 局部搜索后期加速 10-100x

---

## 超参数经验总结

| 问题 | λ 推荐 | stag_limit | 关键洞察 |
|------|--------|------------|----------|
| tour50 | 5000+ | 高 (800) | 小问题可暴力搜索 |
| tour250 | 500 | 400 | 平衡 |
| tour500 | 200 | 300 | 代数充足，重点是多样性 |
| tour750 | **200** ⭐ | 200 | λ=150 会退步 |
| tour1000 | 100 | 150 | 需要进一步测试 |
